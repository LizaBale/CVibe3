...
markedScript.onload = () => console.log('Markdown parser loaded');
...
async function startNewConversation() {
    ...
    try {
        ...
        welcomeMessageDiv.textContent = "Hello👋, how can I assist you today?";
        messagesContainer.appendChild(welcomeMessageDiv);

        // ✅ Only display server response if it contains a non-empty output
        const responseData = await response.json();
        const serverMessage = Array.isArray(responseData) ? responseData[0]?.output : responseData?.output;

        if (serverMessage && serverMessage.trim() !== "") {
            const botMessageDiv = document.createElement('div');
            botMessageDiv.className = 'chat-message bot';
            botMessageDiv.innerHTML = marked.parse(serverMessage || '');
